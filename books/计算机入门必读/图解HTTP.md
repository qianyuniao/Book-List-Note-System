...
著有《今晚我们一起学习邮件协议》（今夜わかゐメールプロトコル）《今晚我们一起学习TCP/IP》（今夜打力わかゐTCP/IP）《今晚我们一起学习HTTP》（今夜わかゐHTTP）。
...
HTTP keep-alive或HTTP connection reuse）的方法。持久连接的特点是，只要任意一端没有明确提出断开连接，则保持TCP连接状态。
...
HttpOnly属性Cookie的HttpOnly属性是Cookie的扩展功能，它使JavaScript脚本无法获得Cookie。其主要目的为防止跨站脚本攻击（Cross-site scripting,XSS）对Cookie的信息窃取。
